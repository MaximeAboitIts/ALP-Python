...
def controler_le_stock(stock, catalogue):
    ValError = 0  # Définis à 0
    for i in range(len(stock)):
        # Définition d'une True / False
        ValExiste = False
        ValNbFois = 0  # Définis à 0

        for z in range(len(stock)):
            # Variable "stock de i" comparé à variable "stock  de z"
            if stock[i] == stock[z]:
                ValNbFois = ValNbFois + 1

        # Indiquer des erreurs -> MULTIPLE ou INCONNU
        for x in range(len(catalogue)):
            if stock[i] == catalogue[x]:
                ValExiste = True
                if ValNbFois > 1:  # Des erreurs -> MULTIPLE ou INCONNU
                    ValError = ValError + 1
                    afficher_msg(MULTIPLE, stock[i], i)
                    # print("Cellule d'indice ", position, ", l'article n°", valeur, sep='', end='')
                    # position = i ; stock[i] = valeur
        if ValExiste == False:  # Des erreures -> INCONNU
            afficher_msg(INCONNU, stock[i], i)
            # print("Cellule d'indice ", position, ", l'article n°", valeur, sep='', end='')
            # position = i ; stock[i] = valeur
            ValError = ValError + 1
        # Indiquer aucune erreur -> OK
    if ValError == 0:  # Aucune erreur -33 > OK
        # len(stock) = Nombre d'article finaux dans la liste au vu de la longeur
